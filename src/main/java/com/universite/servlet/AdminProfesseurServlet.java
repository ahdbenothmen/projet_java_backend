...
@WebServlet("/api/admin/professeurs/*")
public class AdminProfesseurServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        StringBuilder sql = new StringBuilder(
            ...
            "       p.diplomes, p.speciality, p.diplome_pdf " +
            "FROM user u " +
    ...
    byte[] diplomePdf = rs.getBytes("diplome_pdf");
    ...
    json.append("\"diplomes\":\"");        json.append(escape(rs.getString("diplomes")));         json.append("\",");
    ...
    json.append("\"photoCin\":");          json.append(toBase64Json(photoCin,   "image/jpeg"));   json.append(",");
    json.append("\"diplomePdf\":");        json.append(toBase64Json(diplomePdf, "application/pdf"));
    json.append("}");
}
            json.append("]");
...
protected void doPut(HttpServletRequest req, HttpServletResponse resp)
    ...
    // ✅ Fix hint 2 : switch à la place de if/else
    ...
    if (newStatus == null) {
        resp.setStatus(400);
        resp.getWriter().write("{\"error\":\"Action inconnue: " + escape(action) + "\"}");
        return;
    }

    String sqlUpdate = "UPDATE user SET demandestatus = ? WHERE cin = ? AND role = 'PROFESSEUR'";
    String sqlSelect = "SELECT nom, prenom, email FROM user WHERE cin = ?";

    try (Connection conn = DBConnection.getConnection()) {

        // 1. Récupérer infos prof pour l'email
        ...
        if (rows == 0) {
            resp.setStatus(404);
            resp.getWriter().write("{\"success\":false,\"message\":\"Professeur introuvable\"}");
            return;
        }

        // 3. Envoyer email (async pour ne pas bloquer la réponse)
        ...
        resp.getWriter().write(
            "{\"success\":true,\"message\":\"Statut mis à jour: " + newStatus + "\"}"
        );
